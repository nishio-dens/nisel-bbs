...
public class StatusCollectUDPMessageReceiver implements MessageReceiver, Runnable {
	...
	protected StatusCollectUDPMessagingProvider provider;
	private StatusCollectUDPMessageSender sender;
	...
	protected StatusCollectUDPMessageReceiver(InetAddress selfInetAddr, int port, int portRange,
			UDPMessagingConfiguration config, StatusCollectUDPMessagingProvider provider) throws IOException {
		this.config = config;
		...
		this.sender = (StatusCollectUDPMessageSender)this.getSender(true);
	...
	private MessageSender getSender(boolean forReceiver) {
		...
		return new StatusCollectUDPMessageSender(this, forReceiver);
	}
	...
	public void run() {
		...
		while (true) {
			...
			//TODO: ここを変更
			/*System.out.println("Receive Message Size :" + (buf.capacity() -
					buf.remaining()) + "[bytes] from " + srcAddr.getAddress().toString() + ":" 
					+ srcAddr.getPort());*/
			monitor.addMessageLog("RECEIVE_OVERLAY_WEAVER_MESSAGE",
					srcAddr.getAddress().getHostAddress() + ":" + srcAddr.getPort(),
					buf.capacity() - buf.remaining(), Tag.getTagByNumber(msg.getTag()).getName() );
...
//				}
			...
			if (tag == Tag.PUNCH_HOLE_REQ.getNumber()) {		// for UDP hole punching
				...
				if (doHolePunching) {
					...
					if (selfExteriorAddress.equals(selfAddr)) {
						...
						// set self address
						StatusCollectUDPMessageReceiver.this.selfAddr = selfExteriorAddress;

						synchronized (StatusCollectUDPMessageReceiver.this) {
	...
	private class UDPHolePunchingDaemon implements Runnable {
		public void run() {
			try {
				while (true) {
					Thread.sleep(StatusCollectUDPMessageReceiver.this.config.getPunchingCheckInterval());
