...
public class CanBackupHandler implements HttpHandler {

	private Logger logger = Logger.getLogger("bbs.server.handler");
	private BBSManager manager = null;
	
	public CanBackupHandler(BBSManager manager) {
	...
	public void handle(HttpExchange he) throws IOException {
		...
		if( splitStr.length == 5 ) {
			...
			//レスポンス
			boolean res = manager.canBackup(categoryID, topicID);
			//response 200 OK
			...
			logger.info("canbackup request from " + he.getRemoteAddress().getAddress().getHostAddress() +
					" category: " + categoryID + " topic: " + topicID);
		}else {
			//間違った要求 400 Bad Request
			byte[] res = "Bad canbackup Request.".getBytes();
			he.sendResponseHeaders(400, res.length);
			os.write(res);
			
			logger.info("Bad canbackup Request from " + he.getRemoteAddress().getAddress().getHostAddress());
		}
